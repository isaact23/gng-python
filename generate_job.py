...
def generate_job(layer_name):
    ...
    if "dependencies" in layer:
        for dependency in layer["dependencies"]:
            ...
            w.put("\n")
            w.put(point_name + " new_point = point;\n")
            w.put("new_point.localX = point.localX + (chunkX * " + str(CHUNK_WIDTH) + ");\n")
            w.put("new_point.localY = point.localY + (chunkY * " + str(CHUNK_WIDTH) + ");\n")
            w.put("points.Add(new_point);\n")
            w.close_func()
            w.close_func()
            w.put("\n")
